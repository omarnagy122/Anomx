...
def test_consumer_does_not_run_heavy_processing_or_prediction():
    consumer_source = (PROJECT_ROOT / "src" / "ingestion" / "kafka_consumer.py").read_text(encoding="utf-8").lower()
...
def test_prediction_pipeline_reads_incrementally_from_raw_table_not_cmapss_file():
    prediction_source = (PROJECT_ROOT / "src" / "prediction" / "prediction_pipeline.py").read_text(encoding="utf-8").lower()
...
def test_schema_contains_required_tables_and_incremental_keys():
    sql = (PROJECT_ROOT / "infra" / "db" / "init.sql").read_text(encoding="utf-8").lower()
    for table in [
        "raw_sensor_data",
